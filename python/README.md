Project Description: Trip Planner using Uber vs Lyft's Price Estimation
...
Module 1: Using Google Maps API for fetching latitude and logitude for certain location based on name, address, city, 
zip and state and storing the information into a persistent DB which is deployed on AWS RDS instance.
Module 2:Using the location ID, Uber and Lyft products counts around that location can be fetched.
Module 3: based on location ids, a user can Plan a trip for multiple locations and Using Uber and Lyft APIs, it suggests the price estimation
for that trip based on the mot ideal route.

#Module 1: Using Location APIs
